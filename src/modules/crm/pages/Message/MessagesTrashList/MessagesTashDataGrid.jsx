...
function MessagesTashDataGrid() {
  ...
  const [fieldsName] = ['fields.message.'];
  ...
  const handleMessageList = useCallback(async (filters) => {
    return await messageService.getMessageTrashList(filters);
  }, []);
  ...
  return (
    <>
      <MainCard title={t('pages.cards.messagesTrash-list')}>
        <TableCard>
          <MaterialTable
            refetch={refetch}
            columns={columns}
      ...
      <RemoveMessage row={row} open={openDelete} setOpen={setOpenDelete} refetch={handleRefetch} />
    </>
  );
}

export default MessagesTashDataGrid;
